...
async function fetchRelatedImages(keyword) {
    ...
    // Pre-load the first image before displaying
    const firstImage = new Image();
    firstImage.src = data.results[0].urls.full;
    firstImage.onload = () => {
        // Once the first image is loaded, display all related images
        data.results.forEach((result) => {
            ...
            img.addEventListener("click", async () => {
                // Fetch the image and immediately display it
